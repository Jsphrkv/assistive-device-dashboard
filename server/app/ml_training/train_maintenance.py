...
def train_model():
    ...
    print(classification_report(y_test, y_pred, target_names=['OK', 'Needs Maintenance']))
    ...
    # Test prediction
    test_sample = [[50, 1000, 35, 5, 90]]  # Moderate usage
    test_scaled = scaler.transform(test_sample)
    prediction = model.predict(test_scaled)[0]
    prob = model.predict_proba(test_scaled)[0][1]
    
    print(f"\n✓ Test prediction: {'Needs maintenance' if prediction == 1 else 'OK'} (prob: {prob:.2f})")
